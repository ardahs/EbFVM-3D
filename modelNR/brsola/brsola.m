function [problem] = brsola(problem)
% Function  : Broyden's method solver globally convergent
%             Parabolic line search via three point interpolation
%             (Kelley, C.T., Solving Nonlinear Equations with Newton's Method, see item 4)
%
% *** INPUTS **************************************************************************************************************************************************
...
%% Main iteration loop
    while (problem.auxiliar.fnrm > problem.auxiliar.stop_tol && problem.auxiliar.iteration < problem.options.MaxIter && problem.auxiliar.exitflag == 1)
        % Compute iteration step (direction)
            problem = brsola_jbroyden(problem);
        % Compute line search
            problem = brsola_armijo(problem);
        % Update output
